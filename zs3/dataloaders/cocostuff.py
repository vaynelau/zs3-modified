...
class _CocoStuff(data.Dataset):
    ...
    def init_embeddings(self, cfg):
        # embed_arr = load_obj("embeddings/pascal/w2c/norm_embed_arr_" + str(self.w2c_size))
        embed_arr = get_embedding(cfg)
        print('embed_arr', embed_arr.size())
    ...
    def get_embeddings(self, label):
        mask = label == 255
        label[mask] = 0
        lbl_vec = self.embeddings(torch.from_numpy(label).long()).data
